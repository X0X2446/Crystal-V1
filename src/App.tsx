...
//   OUTER iframe  — uses srcDoc (no real URL). The Scramjet SW only intercepts
//                   real navigations, so srcdoc is invisible to it. This layer
//                   injects the adblock script safely before the embed loads.
//
//   INNER iframe  — uses the Scramjet-encoded URL (via ctrl.encodeUrl()).
//                   The SW intercepts this exactly once and proxies the embed.
//
// Key rules:
//   1. We wait for window.__proxyReady before encoding any URL. Without this
//      wait the SW may not be controlling the page yet and the encoded URL
//      falls through to the server's catch-all (returning 404 / index.html).
//   2. The inner iframe must NOT have sandbox set — a sandboxed iframe gets an
//      opaque origin that the SW cannot intercept.
//   3. Never use location.replace() — it escapes the iframe and replaces the
//      top-level window (your React app) with the proxied page.
// ---------------------------------------------------------------------------
function PlayerFrame({
  ...
  // Wait for Scramjet to finish initialising before we encode the URL.
  // index.html dispatches "proxy-ready" and sets window.__proxyReady = true
  // once ScramjetController.init() resolves and the SW is active.
  const [proxyReady, setProxyReady] = useState<boolean>(
    () => !!(window as any).__proxyReady
  );
  useEffect(() => {
    if ((window as any).__proxyReady) return;
    const handler = () => setProxyReady(true);
    window.addEventListener("proxy-ready", handler);
    return () => window.removeEventListener("proxy-ready", handler);
  }, []);

  // Only encode once the proxy is confirmed ready; otherwise use the raw URL
  // so the embed still loads directly (graceful fallback).
  const proxiedUrl = useMemo(() => {
    if (proxyReady && typeof (window as any).proxyURL === "function") {
      try { return (window as any).proxyURL(rawUrl) as string; } catch { /* fall through */ }
    }
    return rawUrl;
  }, [rawUrl, proxyReady]);
  ...
  const adblockScript = useMemo(() => {
    ...
    const blJSON = JSON.stringify(bl);
    const level2 = adblock >= 2 ? `window.alert=window.confirm=()=>null;window.print=()=>{};` : "";
    ...
    return `(function(){try{
      const bl=${blJSON};
      window.open=function(u){try{if(bl.some(b=>String(u||'').includes(b)))return null;}catch{}return null;};
      ${level2}${level3}
      const kb=()=>{try{document.querySelectorAll('a[target="_blank"]').forEach(a=>{try{if(bl.some(b=>(a.href||'').includes(b)))a.removeAttribute('target');}catch{}});}catch{}};
      new MutationObserver(kb).observe(document.documentElement||document,{childList:true,subtree:true});
      document.addEventListener('click',e=>{const a=e.target&&e.target.closest&&e.target.closest('a');if(a&&a.href&&bl.some(b=>a.href.includes(b))){e.preventDefault();e.stopPropagation();return false;}},true);
      ${adblock>=3?`let _c=0;setInterval(()=>{try{if(_c++>30)return;for(let i=0;i<60;i++){clearTimeout(i);clearInterval(i);}}catch{}},3000);`:''}
    }catch(e){}})();`;
  }, [adblock]);

  // Show a loading indicator while waiting for the proxy to be ready
  if (!proxyReady) {
    return (
      <div className="absolute inset-0 bg-black flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <div className="size-8 rounded-full border-2 border-white/20 border-t-white animate-spin" />
          <div className="text-xs text-zinc-500">Initialising proxy…</div>
        </div>
      </div>
    );
  }

  // Outer iframe: srcDoc → SW ignores it entirely (no loop).
  // Must keep allow-same-origin so the inner iframe shares the page origin
  // and its requests can be intercepted by the service worker.
  // Inner iframe: NO sandbox — sandboxed iframes get an opaque origin that
  // the SW cannot intercept, causing requests to fall through to the server.
  const srcdoc = `<!doctype html>
<html><head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no">
<style>html,body{margin:0;padding:0;width:100%;height:100%;background:#000;overflow:hidden}iframe{position:absolute;inset:0;width:100%;height:100%;border:0;background:#000}</style>
