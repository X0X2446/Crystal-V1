...
//
// Architecture:
//   OUTER iframe — srcdoc shell. Invisible to the SW (no real URL to intercept).
//                  Runs adblock script, then calls window.parent.__scramjet
//                  .createFrame() to create a properly proxied inner iframe.
//                  Because srcdoc iframes share origin with the parent (when
//                  sandbox includes allow-same-origin), window.parent is
//                  accessible and __scramjet is available.
//
//   INNER iframe — created by ScramjetController.createFrame() which is the
//                  official Scramjet v2 API. frame.go(url) handles all URL
//                  encoding internally — no manual encoding needed, no
//                  encodeUrl() guessing.
//
// Fallback: if __scramjet is not available the embed loads directly.
// ---------------------------------------------------------------------------
function PlayerFrame({
  ...
  // Wait for ScramjetController to finish init() before rendering.
  ...
  // Adblock code runs inside the outer srcdoc iframe
  ...
  // Script that runs inside the srcdoc iframe to create the proxied player.
  // It accesses window.parent.__scramjet (same-origin, so allowed) and calls
  // createFrame() + frame.go(url) — the only correct Scramjet v2 API.
  // Falls back to a plain iframe if the proxy isn't available.
  const proxyScript = `(function(){
    var url = ${JSON.stringify(rawUrl)};
    var ctrl = window.parent && window.parent.__scramjet;
    var el;
    try {
      if (ctrl && typeof ctrl.createFrame === 'function') {
        var frame = ctrl.createFrame();
        el = frame.frame;
        el.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;border:0;background:#000';
        el.allow = 'autoplay *; fullscreen *; encrypted-media *; picture-in-picture *; gyroscope *; accelerometer *';
        el.setAttribute('allowfullscreen','');
        document.body.appendChild(el);
        frame.go(url);
        return;
      }
    } catch(e) { console.warn('[Crystal] createFrame failed:', e); }
    // Fallback: direct embed (no proxy)
  ...
  // key=rawUrl remounts the iframe whenever the server/episode/movie changes
  const srcdoc = `<!doctype html>
<html><head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no">
<style>html,body{margin:0;padding:0;width:100%;height:100%;background:#000;overflow:hidden}</style>
