...
//
// How it works:
//   - The OUTER iframe uses srcdoc — scramjet's service worker only intercepts
//     real URL navigations, so srcdoc is completely invisible to it. No loop.
//   - Inside that srcdoc we inject the adblock script, then render an INNER
//     <iframe src=proxiedUrl>. Scramjet intercepts that inner src exactly once
//     and serves the proxied embed. Done.
//   - location.replace() is intentionally NOT used — it escapes the iframe
//     and navigates the top-level window (your React app), which is what was
//     causing the homepage to appear instead of the video.
// ---------------------------------------------------------------------------
function PlayerFrame({
  ...
  // Outer iframe uses srcdoc → invisible to scramjet service worker (no loop).
  // Inner iframe uses the already-proxied URL → scramjet handles it once.
  // We do NOT use location.replace() — that escapes the iframe and navigates
  // the top window, replacing your React app with the proxied page.
...
></iframe>
  ...
  }, [proxiedUrl, adblockScript, sandbox]);
