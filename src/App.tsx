...
//
// Architecture:
//   OUTER iframe  — uses srcDoc (no real URL). The Scramjet service worker
//                   only intercepts real fetch/navigate requests, so srcdoc
//                   content is completely invisible to it. This layer exists
//                   purely to inject the adblock script safely.
//
//   INNER iframe  — inside the srcdoc HTML, pointing at the already-encoded
//                   /scramjet/<base64> URL. The SW intercepts this request
//                   exactly once, decodes it, and proxies the real embed.
//
// Key rules that prevent the "homepage shows instead of video" bug:
//   1. The OUTER iframe must NOT have `sandbox` set too tightly — it needs
//      `allow-same-origin` so the inner iframe's fetch goes through the SW
//      (service workers only run on same-origin requests).
//   2. The INNER iframe must NOT have `sandbox` at all (or must include
//      `allow-same-origin`) — sandboxed iframes get a unique opaque origin
//      that the SW cannot intercept, so the request falls through to the
//      server's catch-all which returns index.html.
//   3. Never use location.replace() — it navigates the TOP window, not the
//      iframe, replacing your React app with the proxied page.
// ---------------------------------------------------------------------------
function PlayerFrame({
  ...
  // IMPORTANT: The inner iframe must NOT be sandboxed (or must keep
  // allow-same-origin). A sandboxed iframe gets an opaque origin — the SW
  // cannot intercept opaque-origin requests, so they fall through to the
  // Express catch-all and you get index.html (your homepage) instead of video.
  const srcdoc = useMemo(() => {
    const safeUrl = proxiedUrl.replace(/"/g, "&quot;");
    return `<!doctype html>
<html><head>
<meta charset="utf-8">
...
></iframe>
  ...
  // Outer iframe: srcDoc so SW ignores it entirely.
  // Must include allow-same-origin so inner iframe shares the page origin
  // and its requests are interceptable by the SW.
  // Do NOT pass the user's sandbox toggle to the outer iframe here —
  // sandbox on the outer frame would give it an opaque origin too.
  return (
    <iframe
      key={proxiedUrl}
      srcDoc={srcdoc}
